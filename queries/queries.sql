...
FROM asignatura a
JOIN alumno_se_matricula_asignatura asm ON asm.id_asignatura = a.id
JOIN persona p ON p.id = asm.id_alumno
JOIN curso_escolar c ON c.id = asm.id_curso_escolar
...
ORDER BY departamento, per.apellido1, per.apellido2, per.nombre;
